...
public class CybeFactory : MonoBehaviour
{
    [Header("프리펩과 위치")]
    ...
    // 자료 구조들
    private Queue<GameObject> matarialQueue = new Queue<GameObject>();                              // 원료 입고 큐
    ...
    private List<WorkRequest> requestsList = new List<WorkRequest>();                               // 요청서 리스트
    ...
    void AddMaterial()
    {
        ...
        Debug.Log($"{randomType} 완료 도착, 큐 대기 : {matarialQueue.Count} 개");
    }
   ...
   void ProcessRequests()
    {
        ...
        // 첫번째 요청서 처리 (리스트 순서대로)
        WorkRequest firstRequest = requestsList[0];
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(10, 100, 250, 20), $"완제품 (Dict) : {products[ProductType.chair]} 개 대기");
        GUI.Label(new Rect(10, 140, 250, 20), $"요청서 (List) : {requestsList.Count} 개 대기");
        ...
        GUI.Label(new Rect(300, 140, 150, 20), "R : 요청서 추가");
