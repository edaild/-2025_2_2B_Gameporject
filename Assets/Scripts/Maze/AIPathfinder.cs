...
public class AIPathfinder : MonoBehaviour                                       //BFS를 상용해서 AI가 자동으로 목표까지 이동
{
    [Header("AI 설정")]
    ...
    void Update()
    {
        ...
        {
            StartPathfinding();                                     // 스페이스 바로 AI 자동 생성
        }

        if (Input.GetKeyDown(KeyCode.R))                            // R 키 누르면 리셋
        {
            ResetPosition();                                    
        }
        if (ismoving)                                               // AI 경로 이동에 따라 하게 한다.
        {
            MoveAlongPath();
        }
    }
    ...
    List<MazeCell> GetAccessibleNeighbors(MazeCell cell)
    {
        ...
        // 위
        if (cell.x < gen.height && !cell.topWall.activeSelf)
            neighbors.Add(gen.GetCell(cell.x, cell.z + 1));
    ...
    {
        foreach(MazeCell coll in currentPath)
        {
            coll.SetColor(pathPreviewColor);
        }
    }
