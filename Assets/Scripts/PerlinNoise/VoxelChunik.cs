...
public class VoxelChunik : MonoBehaviour
{
    [Header("청크 설정")]
    public int chunkSize = 16;
    public int chunkHeight = 64;

    [Header("perlin Noiso 설정")]
    ...
    void SetupMesh()
    {
        ...
        {
            Debug.LogWarning("VertexColor 쉐이더를 찾을수 없습니다.");
    ...
    public void GenerateChunk()
    {
        ...
        for(int x = 0; x < chunkSize; x++)
        {
            for(int z = 0; z < chunkHeight; z++)
            {
                ...
                {
                    // 동굴 생성
                    bool isCave = IsCaveAt(worldZ, y, worldZ);

                    if(y == 0)
                    {
                        blocks[x, y, z] = BlockType.Bedrock;           // 맨 아래는 기반
    ...
    // 한 면 추가
    void AddFace(int x, int y, int z, Vector3 direction, Color color, List<Vector3> vertices, List<int> traingles, List<Color> coolors)
    {
    ...
    // 블럭의 보이는 면만 추가
    void AddBlockFaces(int x, int y, int z, BlockType block, List<Vector3> vectors, List<int> triangles, List<Color> colors)
    {
        BlockData blockData = new BlockData(block);

        if(IsTransparent(x, y + 1, z))          // 위
        {
            AddFace(x, y, z, Vector3.up, blockData.blockColor, vectors, triangles, colors);
        }

        if (IsTransparent(x, y - 1, z))          // 아래
        {
            AddFace(x, y, z, Vector3.down, blockData.blockColor, vectors, triangles, colors);
        }

        if (IsTransparent(x, y, z + 1))          // 앞
        {
            AddFace(x, y, z, Vector3.forward, blockData.blockColor, vectors, triangles, colors);
        }

        if (IsTransparent(x, y, z - 1))          // 뒤
        {
            AddFace(x, y, z, Vector3.back, blockData.blockColor, vectors, triangles, colors);
        }

        if(IsTransparent(x + 1, y, z))          // 오른쪽
        {
            AddFace(x, y, z, Vector3.right, blockData.blockColor, vectors, triangles, colors);
        }

        if(IsTransparent(x - 1, y, z ))          // 왼쪽
        {
            AddFace(x, y, z, Vector3.left, blockData.blockColor, vectors, triangles, colors);
        }
    }
            ...
            for (int y = 0; y < chunkSize; y++)
            {
                for(int z = 0; z < chunkSize; z++)
                {
                   BlockType block = blocks[x, y, z];
                    ...
                    AddBlockFaces(x, y, z, block, vectors, triangles, colors);      // 6 면 체크
                }
            }
        }

        chunkMesh.Clear();
        chunkMesh.vertices = vectors.ToArray();
