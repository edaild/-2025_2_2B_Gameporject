...
public class CityGenerator : MonoBehaviour
{
    ...
    {
        int floor = Random.Range(minBuildingHeight, maxBuildingHeight);
        float height = floor * floorHeight;
    ...
    void CreateTower(int x, int z)
    {
        ...
        topLight.transform.position = new Vector3(x * blockSize, height + 3f, z * blockSize);
    ...
    void CreatePark(int x, int z)
    {
        ...
        park.transform.localScale = new Vector3(blockSize * 0,2f, blockSize * 0.8f);
        ...
        for(int i = 0; i < 3; i++)
        {
            Vector3 treePos = new Vector3(
                ...
                blockSize + Random.Range(-blockSize * 0.3f, blockSize * 0.3f)
                );
    ...
    void CreatePlaza(int x, int z)
    {
        ...
        plaza.transform.localScale = new Vector3(blockSize * 0, 2f, blockSize * 0.8f);
        plaza.transform.SetParent(transform);
        plaza.GetComponent<Renderer>().material.color = new Color(0.9f, 0.9f, 0.85f);

        GameObject fountain = GameObject.CreatePrimitive(PrimitiveType.Cube);
    ...
    void CreateSteetLight(int x, int z)
    {
    ...
    {
        float disFromCentor = Vector2.Distance(new Vector2(x, z), new Vector2(cityWidth/2f, cityHeight/2f));

        if(disFromCentor < cityWidth * 0.2f)
        {
            return new Color(0.6f, 0.6f, 0.7f);
        }
        else if(disFromCentor < cityWidth * 0.4f)
        {
